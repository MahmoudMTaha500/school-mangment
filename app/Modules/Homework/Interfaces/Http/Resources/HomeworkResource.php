...
final class HomeworkResource extends JsonResource
{
    ...
    {
        return ['id' => $this->id, 'class_section_id' => $this->class_section_id, 'subject_id' => $this->subject_id, 'teacher_id' => $this->teacher_id, 'title' => $this->title, 'body' => $this->body, 'due_at' => $this->due_at?->toISOString(), 'submissions_count' => $this->whenCounted('submissions'), 'submissions' => SubmissionResource::collection($this->whenLoaded('submissions')), 'created_at' => $this->created_at?->toISOString()];
    }
}
