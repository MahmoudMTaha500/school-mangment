...
export interface ApiClientOptions {
    baseUrl: string;
    token?: string | null;
    /** Injectable for tests; defaults to the global fetch (available in RN). */
    fetchFn?: typeof fetch;
}

/**
 * Framework-agnostic typed client. Deliberately free of any react-native import
 * so it can be unit-tested in Node and shared across every screen.
 */
export class ApiClient {
    ...
    async login(path: '/auth/login', email: string, password: string, deviceName = 'mobile'): Promise<string> {
    ...
    async delete(path: string, payload?: unknown): Promise<void> {
        await this.request<null>('DELETE', path, payload);
    }
